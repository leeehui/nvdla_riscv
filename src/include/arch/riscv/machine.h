...
#define read_set_csr(reg, val) ({ unsigned long __tmp; \
  asm volatile ("csrrs %0, " #reg ", %1" : "=r"(__tmp) : "rK"(val)); __tmp; })

#define read_clear_csr(reg, val) ({ unsigned long __tmp; \
  asm volatile ("csrrc %0, " #reg ", %1" : "=r"(__tmp) : "rK"(val)); __tmp; })
